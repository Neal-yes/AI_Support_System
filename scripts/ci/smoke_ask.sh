...
validate_rag() {
  ...
  # meta.use_rag must be true; when COLLECTION is provided, response .meta.collection must match it
  # Note: sources can be empty in smoke; treat empty sources as non-blocking to reduce flakiness
  if ! jq -e --arg coll "$COLLECTION" '
    (.meta.use_rag == true)
    and (((($coll | length) == 0)) or (.meta.collection == $coll))
  ' "$file" >/dev/null 2>&1; then
    echo "[SMOKE] rag: require meta.use_rag=true, and when requested collection is set, meta.collection must match it" >&2; return 1
  fi
  return 0
}
